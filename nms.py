...
def overlapping_area_HOG(detection_1, detection_2):
...
def nms_HOG(detections, threshold=.5):
    ...
    #     for new_detect in new_detections:
    #         if (overlapping_area(detections[index], new_detections) > threshold):
    #             del detections[index]
    #             break
    #         else:
    ...
    for index, detection in enumerate(detections):
        for new_detection in new_detections:
            if overlapping_area_HOG(detection, new_detection) > threshold:
                del detections[index]
                break
        else:
            new_detections.append(detection)
